...
def test_pivot(n, max_l, max_k):
    ...
    for _ in range(n):
        ...
        for j in range(l):
            ...
            if (
                a[ind] != p
                or any([j > p for j in a[left:ind]])
                or any([j < p for j in a[ind : right + 1]])
                or any(a[j] != a_orig[j] for j in range(0, left))
                or any(a[j] != a_orig[j] for j in range(right + 1, len(a)))
            ):
