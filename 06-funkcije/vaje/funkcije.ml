...
let rec map f list =
  let rec pomozna_map acc f list = 
  match list with
    | [] -> acc
    | x :: xs -> pomozna_map ((f x) :: acc) f xs
  in
  reverse (pomozna_map [] f list)
...
let rec mapi f list =
  let rec mapi_pomozna acc index f list =
    match list with
    | [] -> acc
    | x :: xs -> mapi_pomozna ((f x index) :: acc) (index + 1) f xs
  in
  reverse (mapi_pomozna [] 0 f list)
